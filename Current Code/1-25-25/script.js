let questionseng = [
    ...
    [
        ...
        ["Which sentence is considered grammatically correct?"]
    ],
    [
    ...
    [
        ["Which sentence below is presented in passive voice?"], 
        ["Which of the following below is a compound sentence?"]
    ],
    [
...
let correct = 1;
let selected = 0;
let subject = (JSON.parse(localStorage.getItem("subject")) || "ELA").toUpperCase();
let prev_window = "";
let fin = 0;
...
function medallion_offset(medallion) {
    let number = Math.floor(Math.random() * (80 - 20 + 1)) + 20;
    document.getElementById(medallion).style.marginLeft = number.toString() + "%";
...
function change_question(medallion, question) {
    ...
    if (window.location.pathname.includes("index.html")) {
        ...
        localStorage.setItem("subject", JSON.stringify(subject.toUpperCase()));
        localStorage.setItem("prev_window", JSON.stringify("/index.html"));
        window.location.href = window.location.href.replace("/index.html", "/practice.html");
    }
...
function change_subject(state) {
    ...
    localStorage.setItem("subject", JSON.stringify(subject.toUpperCase()));
    document.getElementById("course").textContent = subject.toUpperCase();
}
